...
sed -i 's/rev = 3/rev = 6/g' hw/i386/acpi-build.c # Most VMs use an older-style FADT of length 244  bytes (revision  3), cutting off before the Sleep Control/Status registers and Hypervisor ID
...
sed -i 's/rev = 1/rev = 6/g' hw/i386/acpi-build.c # 全部升级最低为6
