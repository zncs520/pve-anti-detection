...
int kvm_emulate_rdmsr(struct kvm_vcpu *vcpu)
{
	...
	if(ecx==0x4b564d00){r=1;}//强制成不可访问
	if(ecx==0x1db){r=1;}//强制成不可访问
	if(ecx==0x1a2){r=0;}//强制成可访问
	if(ecx==0x19c){r=0;}//强制成可访问

	if (!r) {
		if(ecx==0x1d9){//1D9H 473 IA32_DEBUGCT
			data=data_1d9;//dds666直接读取缓存数据
			//printk("dds666 rdmsr 0x1d9 return data=%llu\n",data);
		}
		//if(ecx==0x179){data=0xc14;}//dds666 强制改数据 提供机器检查架构 (MCA) 的功能和限制信息，用于处理硬件错误和异常。
		//if(ecx==0x3a){data=0x5;}//dds666 强制改数据 用于控制处理器的特性和功能，例如启用或禁用虚拟化支持。
		//if(ecx==0x198){data=0x258d00002f00;}//dds666 强制改数据，提供有关处理器当前性能状态的信息，包括频率、电压和功耗等。
		//if(ecx==0x199){data=0x3d00;}//dds666 强制改数据，用于控制处理器的性能和功耗管理，可以调整处理器的频率和电源状态。
		//if(ecx==0x1a2){data=0x641400;}//dds666 强制改数据 寄存器的bit23-16记录了TCC_ACTIVATION_TEMP值，改值可理解为CPU触发调温手段的阈值，该阈值出厂时设置；
		//if(ecx==0x1a0){data=0x850089;}//dds666 强制改数据 
		//if(ecx==0x19c){data=0x88490000;}//dds666 强制改数据 寄存器的bit22-16记录了 TEMPERATURE—值，这个值是CPU温度相对于TCC_ACTIVATION_TEMP的温度值；
		//if(ecx==0x606){data=0xa0e03;}//dds666 强制改数据 606H 1542 MSR_RAPL_POWER_UNIT
		//if(ecx==0x1fc){data=0xfc005b;}//dds666 强制改数据 1FCH 508 MSR_POWER_CTL
...
int kvm_emulate_wrmsr(struct kvm_vcpu *vcpu)
{
	...
	int r;
	if(ecx==0x1d9){
		if(data==0x4000||data==0x4001||data==0x4002||data==0x4003){
			r=0;//直接不屏蔽
		}
	}else{
	...
	if (!r) {
		if(ecx==0x1d9){//1D9H 473 IA32_DEBUGCT
			if(data==0x4000||data==0x4001){data_1d9=0x4000;
				printk("dds666 wrmsr 0x1d9 data=%llx data_1d9=%llx\n",data,data_1d9);
